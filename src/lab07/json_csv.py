...
def json_to_csv(json_path: str, csv_path: str) -> None:
    ...
    # Предупреждение, если были не-словари
    non_dict_count = len(data) - len(dict_data)
    if non_dict_count > 0:
        print(f"Предупреждение: пропущено {non_dict_count} элементов, которые не являются словарями")
    
    if len(dict_data) == 0:
        raise ValueError("В JSON файле нет словарей для преобразования")
